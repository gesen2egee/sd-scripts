...
def test_sample_training_noise_uses_gaussian_when_knn_noise_k_is_zero(monkeypatch):
    latents = torch.zeros(2, 4, 8, 8, dtype=torch.float32)
    expected = torch.ones_like(latents)
    args = argparse.Namespace(knn_noise_k=0)
...
def test_apply_immiscible_image_scale_multiplies_latents():
    latents = torch.tensor([[[[1.0, -2.0]]]], dtype=torch.float32)
    args = argparse.Namespace(immiscible_image_scale=2.0)

    scaled = train_util.apply_immiscible_image_scale(args, latents)

    assert torch.equal(scaled, latents * 2.0)
    assert scaled.dtype == latents.dtype
    assert scaled.device == latents.device


def test_verify_training_args_rejects_invalid_immiscible_image_scale():
    args = _build_verify_args(immiscible_image_scale=0.5)
    with pytest.raises(ValueError, match="immiscible_image_scale"):
