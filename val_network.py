...
class NetworkTrainer:
    ...
    def get_noise_pred_and_target(
        ...
        huber_c = None
        target = noise
        timesteps = torch.full((b_size,), fixed_timestep, device="cpu")
    ...
    def train(self, args):
        ...
        for epoch in range(epoch_to_start, num_train_epochs):
            ...
            for step, batch in enumerate(skipped_dataloader or train_dataloader):
                ...
                with accelerator.accumulate(training_model):
                    ...
                    current_loss = average_loss.detach().item()
                    files_to_move.append((batch["images_paths"][0], current_loss))
                    if current_loss > 0.1:
                        print(batch["captions"][0], current_loss)                
        ...
        files_to_move = sorted(files_to_move, key=lambda x: x[1], reverse=True) 
        for file_path, current_loss in files_to_move[:int(len(files_to_move) * 0.2)]:
                try:
                    file_root, file_ext = os.path.splitext(file_path)
                    target_folder = os.path.join(os.path.dirname(file_path), str(int(current_loss * 100)))
                    os.makedirs(target_folder, exist_ok=True)                    
                    shutil.move(file_path, os.path.join(target_folder, os.path.basename(file_path)))                    
                    for related_file in os.listdir(os.path.dirname(file_path)):
                        try:
                            if related_file.startswith(os.path.basename(file_root)) and not related_file.lower().endswith(('.png', '.jpg', '.jpeg', '.webp', '.bmp')):
                                shutil.move(os.path.join(os.path.dirname(file_path), related_file), target_folder)
                        except Exception as e:
                            print(f"Failed to move related file: {related_file}. Error: {e}")
                except Exception as e:
